...
class Tweetify extends UserManager implements TweetManager, FiturTweet
{
    function getInfoApp()
    {
    ...
    function like($data)
    {
        if (isset($_POST["like-tweet"])) {
            $id = $data["id"];
            $user_id = $data["user_id"];

            // Cek apakah user sudah melakukan "like" sebelumnya
            $query = "SELECT * FROM likes WHERE tweet_id = '$id' AND user_id = '$user_id'";
            $result = mysqli_query($this->getConn(), $query);

            // Jika user sudah melakukan "like", maka hapus "like"
            if (mysqli_num_rows($result) > 0) {
                mysqli_query($this->getConn(), "DELETE FROM likes WHERE tweet_id = '$id' AND user_id = '$user_id'");

                // if (mysqli_affected_rows($this->getConn()) == 1) {
                //     echo "<script>
                // alert('Like berhasil dihapus!');
                // </script>";
                // } else {
                //     echo "<script>
                // alert('Gagal menghapus like: " . mysqli_error($this->getConn()) . "');
                // </script>";
                //     return false;
                // }

                // Mengembalikan status "like" yang dihapus
                return false;
            } else {
                // Jika user belum melakukan "like", maka tambahkan "like"
                mysqli_query($this->getConn(), "INSERT INTO likes (user_id, tweet_id) VALUES ('$user_id', '$id')");

                // if (mysqli_affected_rows($this->getConn()) == 1) {
                //     echo "<script>
                // alert('Like berhasil ditambahkan!');
                // </script>";
                // } else {
                //     echo "<script>
                // alert('Gagal menambahkan like: " . mysqli_error($this->getConn()) . "');
                // </script>";
                //     return false;
                // }

                // Mengembalikan status "like" yang ditambahkan
                return true;
            }
        }
    }
    ...
    function displayLike($data)
    {
        if (isset($_POST["like-tweet"])) {
            $id = $data["id"] ?? null;
            $query = "SELECT COUNT(*) AS total FROM Likes WHERE tweet_id = $id";
            $result = mysqli_query($this->getConn(), $query);
